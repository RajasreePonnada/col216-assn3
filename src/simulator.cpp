...
void Simulator::run() {
    ...
    while (true) {
        ...
        if (global_cycle > 5000000000) {
         ...
         if (global_cycle % 5000000 == 0) { // Progress indicator every 5M cycles
...
void Simulator::printStats() {
    statistics.printFinalStats(block_size);
...
cycle_t Simulator::getMaxCycles() const {
     ...
     cycle_t max_c = 0;
     if (statistics.total_cycles.empty()) return 0; // Should not happen
